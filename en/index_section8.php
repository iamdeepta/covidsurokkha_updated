...
<div class="main_title text-center">';
  /*if($lastSegment=='en'){
$output .= '<h5 id="hospitals_nearby_text" style="font-size: 38px;">Our Blogs</h5>';
}else{
  $output .= '<h5 class="notranslate" id="hospitals_nearby_text" style="font-size: 38px;">আমাদের ব্লগ</h5>';
}*/

$output .= '<h2 style="color:#01cfbe;">Our Blogs</h2>';
...
<div class="owl-carousel">';
  ...
  while($blogs_query && $result=mysqli_fetch_array($blogs_query, MYSQLI_ASSOC)){
   ...
   $output .= '<div id="blogs_card_main_div" style="padding:20px 20px;">
    ...
    <div class="card blogs_card" id="blogs_card'.$result['blogs_id'].'" style="width: 22vw;border-radius: 15px;border: 0px transparent;background-color: rgb(153 243 239 / 15%);cursor:pointer">
      ...
      <div id="blogs_date_div" style="position: absolute;z-index: 99;background-color: #01cfbe;padding:2px 10px;border-radius: 8px;margin-top: -17px;left: calc(100% - 60px);width: 70px;text-align: center;display: flex;justify-content: center;flex-direction: row;flex-wrap: wrap;margin-right: 30px;float:left"><span style="color: white">'.$result['blogs_date'].'</span></div>

      <div id="blog_tag_div" style="position: absolute;z-index: 99;background-color: #2eaba0;padding:2px 15px;border-radius: 8px;top: 182px;left: calc(100% - 160px);width: 140px;text-align: center;display: flex;justify-content: center;flex-direction: row;margin-right: 30px;flex-wrap: wrap;float:left">';

      if($result['blogs_tag_en']!=''){
      $output .= '<span id="blog_tag_text" style="color: white">'.$result['blogs_tag_en'].'</span>';
  		}else{
  			$output .= '<span id="blog_tag_text" style="color: white">'.$result['blogs_tag_bn'].'</span>';
  		}
      ...
      <div class="card-body" style="border-color: transparent;margin-top: 10px">';
       ...
       if($result['blogs_description1_en']!=''){
        ...
        $output .= '<p style="margin-top: 10px;font-style: italic;font-size: 14px"><a href="" style="text-decoration: none;color: #58547e;">'.$result['blogs_category_en'].'</a></p>';
    		...
    		$output .= '<p style="margin-top: 10px;font-style: italic;font-size: 14px"><a href="" style="text-decoration: none;color: #58547e;">'.$result['blogs_category_bn'].'</a></p>';
    	}

        $output .= '<span style="margin-top: 10px;font-size: 14px"><i class="far fa-clock"></i><span style="margin-left: 3px">'.$result['blogs_time'].' minute read</span></span>
        </div>
  ...
  $("#blogs_card'.$result['blogs_id'].'").click(function(){
    ...
    window.location.href="blogs-detail.php?id='.$result['blogs_id'].'&lang=en";
  });
    ...
    /*$output .= '<div id="contextMenu'.$result['blogs_id'].'" class="context-menu" 
        style="display:none">
        <ul>
            <li><a href="blogs-detail.php?id='.$result['blogs_id'].'&lang=en" target="_blank" style="font-size:14px">Open link in new tab</a></li>
  ...
  $output .= '</div></div>';
    ...
    $output .= '<div class="container" id="btn_view_all_div_mbl" style="display:none">
                <div>
                  <p style="font-size:14px;margin-top:16px">Swipe to browse latest blogs</p>
                </div>
                <div>
                  <a href="blogs.php"><button id="btn_view_all_mbl">View All</button></a>
                </div>
              </div>';

  $output .= '<div id="btn_view_all_div"><a href="blogs.php"><button id="btn_view_all">View All</button></a></div>';
